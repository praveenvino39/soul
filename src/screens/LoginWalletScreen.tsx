...
type LoginWalletScreenProp = NativeStackNavigationProp<RootStackParamList, 'LoginWalletScreen'>;

function LoginWalletScreen() {
    const { createWalletWithFaceId } = useEVMWallet()
    const { setWallet } = useContext(EvmWalletContext)
    const { navigate } = useNavigation<LoginWalletScreenProp>()
    ...
    return (
        <>
            <WebView
                ...
                source={{ uri: "https://f5v7zf.csb.app/login" }}
